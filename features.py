...
def load_data(path):
    ...
    df = df.sort_values(['season','round_num','tier','match_id']).reset_index(drop=True)
    return df
...
def compute_rolling_form(df, window=FORM_WINDOW):
    ...
    for h_col,a_col,f_name,a_name,_ in STAT_PAIRS:
        rename_home[h_col] = f_name
        rename_home[a_col] = a_name
        rename_away[a_col] = f_name
        rename_away[h_col] = a_name

    stat_src_cols = [p[0] for p in STAT_PAIRS] + [p[1] for p in STAT_PAIRS]
    home_part = df[['season','round_num','home_team_name'] + stat_src_cols].rename(columns=rename_home)
    home_part['orig_index'] = df.index
    home_part['is_home'] = 1
    away_part = df[['season','round_num','away_team_name'] + stat_src_cols].rename(columns=rename_away)
    ...
    long_df = long_df.sort_values(['team','season','round_num']).reset_index(drop=True)
    grp = long_df.groupby('team', group_keys=False)

    fill_vals = {'points':1.35}
    for h_col,a_col,f_name,a_name,default in STAT_PAIRS:
        fill_vals[f_name] = default
        fill_vals[a_name] = default

    stat_cols = list(fill_vals.keys())
    for col in stat_cols:
        long_df[f'form_{col}'] = grp[col].apply(lambda s: s.shift(1).rolling(window,min_periods=1).mean())
    long_df = long_df.fillna({f'form_{k}':v for k,v in fill_vals.items()})
    ...
    df = df.join(home_rows).join(away_rows)
    return df
...
def compute_h2h(df, window=H2H_WINDOW):
    ...
    h2h_vals = pd.Series(index=df.index, dtype=float)
    for key, grp in df.groupby('pair_key'):
        grp_sorted = grp.sort_values(['season','round_num'])
        rolled = grp_sorted['home_win_flag'].shift(1).rolling(window,min_periods=1).mean()
        h2h_vals.loc[grp_sorted.index] = rolled.values
    df['h2h_home_rate'] = h2h_vals.fillna(0.45)
    df = df.drop(columns=['pair_key','home_win_flag'])
    return df

def compute_prev_tier(df):
    long_home = df[['season','home_team_name','tier']].rename(columns={'home_team_name':'team'})
    long_away = df[['season','away_team_name','tier']].rename(columns={'away_team_name':'team'})
    long_all = pd.concat([long_home, long_away], ignore_index=True)
    season_tier = long_all.groupby(['team','season'])['tier'].min().reset_index()
    season_tier['season'] = season_tier['season']+1
    season_tier = season_tier.rename(columns={'tier':'prev_tier'})
    df = df.merge(season_tier, left_on=['home_team_name','season'], right_on=['team','season'], how='left').drop(columns=['team'])
    df = df.rename(columns={'prev_tier':'home_prev_tier'})
    df = df.merge(season_tier, left_on=['away_team_name','season'], right_on=['team','season'], how='left').drop(columns=['team'])
    df = df.rename(columns={'prev_tier':'away_prev_tier'})
